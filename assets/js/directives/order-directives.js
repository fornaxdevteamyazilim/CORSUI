                ...
                if (item.Options) {
                    for (var i = 0; i < item.Options.length; i++) {
                        for (var a = 0; a < item.Options[i].Items.length; a++) {
                            ...
                            if (item.Options[i].OptionType == 3) {
                                item.Options[i].Items[a].name = (item.Options[i].Items[a].Price&&item.Options[i].Items[a].Price.Price>0) ? item.Options[i].Items[a].name + ' ' + item.Options[i].Items[a].Price.Price : item.Options[i].Items[a].name;
                            }
...
function ngCheckbox($rootScope) {
    return {
        ...
        //template:"<div ng-click='toggle()' class='col-lg-12'><div class='col-lg-1' style='text-align: left;'> {{editorValue.Quantity| number:0}}</div> <div class='col-lg-9' style='overflow-y: hidden;height: 20px;' >{{:: item.name}}</div><div class='col-lg-2' style='text-align: left;'> {{:: editorValue.Price.Price| number:2}}</div></div></label>" ,
        replace: true,
        require: 'ngModel',
